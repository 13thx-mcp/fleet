...
- keep source-controlled host examples in `hosts/*.example.toml` only;
- load active host profiles only from `mcp-server/runtime/fleet/hosts/<host>.toml`;
...
`hosts/host.example.toml` is a source-controlled template, never an active profile.
Each machine owns its active profile at `mcp-server/runtime/fleet/hosts/<host>.toml`.
That file contains host identity, absolute machine paths, tunnel identity, and only a
reference to the control-plane API-key file. It is not copied into, generated by, or
read from source control. `fleetctl --host <host>` resolves this runtime path in both
the source checkout and deployed runtime Fleet bundle.
...
`deploy-control` copies the Fleet manifest, documentation, and `fleetctl.py` into
`mcp-server/runtime/fleet`. It preserves the active runtime host profile already in
`runtime/fleet/hosts`; it never copies or overwrites that host-owned file. Source
repositories are resolved from `host.source_root`; Rust MCP binaries are installed
into the flat `host.bin_root`; non-MCP services such as Studio use
`install_scope = "runtime"` and install beneath `host.runtime_root`. Generated
operational state remains under `host.runtime_root`.
...
└── runtime/
    ├── fleet/
    │   └── hosts/<host>.toml  # host-local active profile; never source controlled
    ├── gateway/servers.d/
    ├── studio/
    └── tunnel-client/
        ├── config.yaml
